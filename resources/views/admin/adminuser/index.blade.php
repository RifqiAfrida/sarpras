        ...
        <div class="row">
            ...
            <div class="card my-4">
                ...
                <div class="bg-gradient-info shadow-primary border-radius-lg pt-4 pb-3">
                    <h6 class="text-white text-capitalize ps-3">TABLE DATA ADMIN</h6>
                </div>
                @if (auth()->user()->roles_id == 1)
                <a href="{{ route('admin.adminusers.create') }}" class="btn bg-gradient-success w-100 my-4 mb-2">Create</a>
                @else
                                ...
                                @foreach ($adminuser as $adminuser)
                                    <tr>
                                        <td>
                                            <div class="d-flex px-2 py-1">
                                                <div class="d-flex flex-column justify-content-center">
                                                    <h6 class="mb-0 text-sm">{{ $adminuser->id }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="d-flex px-2 py-1">
                                                <div class="d-flex flex-column justify-content-center">
                                                    <h6 class="mb-0 text-sm">{{ $adminuser->email }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="d-flex px-2 py-1">
                                                <div class="d-flex flex-column justify-content-center">
                                                    <h6 class="mb-0 text-sm">{{ $adminuser->role->name }}</h6>
                                                </div>
                                            </div>
                                        </td>
                                <td class="align-middle">
                                   @if (auth()->user()->roles_id == 1)
                                   <form action="{{ route('admin.adminusers.destroy', $adminuser->id) }}" method="POST" class="d-inline">
                                       @csrf
                                       @method('delete')
